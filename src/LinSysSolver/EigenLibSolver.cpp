...
namespace DOT {
    ...
    template <typename vectorTypeI, typename vectorTypeS>
    void EigenLibSolver<vectorTypeI, vectorTypeS>::update_a(const Eigen::SparseMatrix<double>& mtr)
    {
        if(useDense) {
            coefMtr_dense.setZero();
            for (int k = 0; k < mtr.outerSize(); ++k) {
                ...
                {
                    coefMtr_dense(it.row(), it.col()) = it.value();
                }
            }
        }
        else {
            assert(mtr.nonZeros() == coefMtr.nonZeros());
            memcpy(coefMtr.valuePtr(), mtr.valuePtr(), coefMtr.nonZeros() * sizeof(double));
        }
    }
    ...
    template <typename vectorTypeI, typename vectorTypeS>
    void EigenLibSolver<vectorTypeI, vectorTypeS>::solve_threadSafe(Eigen::VectorXd &rhs,
                                                                    Eigen::VectorXd &result,
                                                                    int dimI)
    {
        solve(rhs, result);
