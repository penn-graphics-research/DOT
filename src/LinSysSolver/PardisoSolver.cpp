...
namespace DOT {
    ...
    template <typename vectorTypeI, typename vectorTypeS>
    void PardisoSolver<vectorTypeI,vectorTypeS>::update_a(const Eigen::SparseMatrix<double>& mtr)
    {
        Eigen::SparseMatrix<double> mtr_lowerLeft = mtr.triangularView<Eigen::Lower>();
        
        assert(Base::numRows == static_cast<int>(mtr_lowerLeft.rows()));
        assert(Base::a.size() == mtr_lowerLeft.nonZeros());
        memcpy(Base::a.data(), mtr_lowerLeft.valuePtr(),
               mtr_lowerLeft.nonZeros() * sizeof(mtr_lowerLeft.valuePtr()[0]));
    }
