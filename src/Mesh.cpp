...
namespace DOT {
    ...
    template<int dim>
    Mesh<dim>::Mesh(const Eigen::MatrixXd& V_mesh, const Eigen::MatrixXi& F_mesh,
    ...
    template<int dim>
    Mesh<dim>::Mesh(Primitive primitive, double size, int elemAmt,
                                    double YM, double PR, double rho)
    {
        assert(dim == 2);
#if(DIM == 2)
        ...
        switch(primitive)
        {
            ...
            case P_RECTANGLE: {
                double spacing = size / std::sqrt(elemAmt);
                assert(size >= spacing);
                int gridSize = static_cast<int>(size / spacing) + 1;
                spacing = size / (gridSize - 1);
                
                Eigen::MatrixXd UV_bnds(gridSize * 3 - 4, 2);
                borderVerts_primitive.resize(2);
                int vI = 0;
                for(int rowI = 0; rowI < gridSize; rowI++)
                {
                    for(int colI = 0; colI < gridSize / 2; colI++)
                    {
                        if((colI > 0) && (colI < gridSize / 2 - 1) &&
                           (rowI > 0) && (rowI < gridSize - 1))
                        {
                            continue;
                        }
                ...
                Eigen::MatrixXi E(gridSize * 3 - 4, 2);
                for(int colI = 0; colI + 1 < gridSize / 2; colI++) {
                    E.row(colI) << colI, colI + 1;
                }
                for(int rowI = 0; rowI + 1 < gridSize; rowI++) {
                    E.row(gridSize / 2 - 1 + rowI) <<
                    borderVerts_primitive[1][rowI],
                    borderVerts_primitive[1][rowI + 1];
                }
                for(int colI = 0; colI + 1 < gridSize / 2; colI++) {
                    E.row(gridSize * 3 / 2 - 2 + colI) << UV_bnds.rows() - 1 - colI, UV_bnds.rows() - 2 - colI;
                }
                for(int rowI = 0; rowI + 1 < gridSize; rowI++) {
                    E.row(gridSize * 2 - 3 + rowI) <<
                    borderVerts_primitive[0][gridSize - 1 - rowI],
                    borderVerts_primitive[0][gridSize - 2 - rowI];
                }
                
                std::string flag("qQa" + std::to_string(size * size / 2.0 / elemAmt * 1.1));
                // "q" for high quality mesh generation
                // "Q" for quiet mode (no output)
                // "a" for area upper bound
            ...
            case P_SHARKEY: {
                ...
                IglUtils::findBorderVerts(V, borderVerts_primitive, 0.01);
    ...
    void Mesh<dim>::computeFeatures(bool multiComp, bool resetFixedV)
    {
        ...
        for(int vI = 0; vI < V_rest.rows(); ++vI) {
            if(vNeighbor[vI].empty()) {
                std::cout << "isolated nodes detected!" << std::endl;
                exit(-1);
            }
        }
        std::cout << "no isolated nodes" << std::endl;
    }
...
//        }
    ...
    template<int dim>
    void Mesh<dim>::constructSubmesh(const Eigen::VectorXi& triangles,
                                             Mesh<dim>& submesh,
                                             ...
                                             std::map<int, int>& globalTriIToLocal) const
    {
        ...
        submesh = Mesh(V_rest_sub, F_sub, V_sub,
                               m_YM, m_PR, density);
        ...
        submesh.m_isBoundaryVert.resize(submesh.V.rows(), false);
        for(int vI = 0; vI < m_isBoundaryVert.size(); ++vI) {
            if(m_isBoundaryVert[vI]) {
                auto finder = globalVIToLocal.find(vI);
                if(finder != globalVIToLocal.end()) {
                    submesh.m_isBoundaryVert[finder->second] = true;
                }
            }
        }
    }
