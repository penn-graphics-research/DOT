...
//        const double normalizer_div = data.surfaceArea;
    ...
    void SymStretchEnergy::computeHessianBySVD(const TriangleSoup& data, Eigen::VectorXd* V,
...
//        for(int triI = 0; triI < data.F.rows(); triI++) {
        tbb::parallel_for(0, (int)data.F.rows(), 1, [&](int triI) {
            ...
            // left term:
            Eigen::Vector2d d2E_div_dsigma2;
            d2E_div_dsigma2[0] = 2.0 + 6.0 * std::pow(svd.singularValues()[0], -4);
            d2E_div_dsigma2[1] = 2.0 + 6.0 * std::pow(svd.singularValues()[1], -4);
