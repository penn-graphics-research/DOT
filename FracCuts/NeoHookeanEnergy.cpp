...
namespace FracCuts {
    ...
    void NeoHookeanEnergy::getEnergyValPerElem(const TriangleSoup& data, Eigen::VectorXd& energyValPerElem, bool uniformWeight) const
    {
        ...
        tbb::parallel_for(0, (int)data.F.rows(), 1, [&](int triI) {
            ...
            Eigen::JacobiSVD<Eigen::MatrixXd> svd(Xt * A); //TODO: only decompose once for each element in each iteration, would need ComputeFull U and V for derivative computations
    ...
    void NeoHookeanEnergy::checkEnergyVal(const TriangleSoup& data) const // check with isometric case
    {
        ...
        for(int triI = 0; triI < data.F.rows(); triI++) {
            ...
            Eigen::JacobiSVD<Eigen::MatrixXd> svd(X0 * A); //TODO: only decompose once for each element in each iteration, would need ComputeFull U and V for derivative computations
